...
def home(request, ):
    return render(request, 'home/main.html',{'page_title':'Home','products':product_list.order_by('?'), 'categories': list(Category.objects.all()),'categories_active': Category.objects.all()[0:3],'categories_rest': Category.objects.all()[3:7] })

def latest_section(request, ):
    return render(request, 'home/latest.html',{'page_title':'Home', 'products':product_list.order_by('-id'), 'categories': Category.objects.all(),'categories_active': Category.objects.all()[0:3],'categories_rest': Category.objects.all()[3:7] })
